...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (method === 'POST') {
      if (type === 'profile') {
        const { name='', subtitle='', hero_badge='', email='', phone='', linkedin='', github='', about_text='', location='', languages='', project_count='', club_count='', current_year='', image_url='', cgpa='', current_focus='', core_philosophy='', contact_text='', resume_url='' } = data || {};
        if (image_url && image_url.length > 3000000) {
          return response.status(400).json({ error: 'Image exceeds 2MB size limit' });
        }
        if (resume_url && resume_url.length > 5000000) {
          return response.status(400).json({ error: 'Resume file exceeds 5MB size limit' });
        }
        const exists = await sql`SELECT * FROM profile LIMIT 1;`;
        if (exists.rows.length > 0) {
          await sql`UPDATE profile SET name=${name}, subtitle=${subtitle}, hero_badge=${hero_badge}, email=${email}, phone=${phone}, linkedin=${linkedin}, github=${github}, about_text=${about_text}, location=${location}, languages=${languages}, project_count=${project_count}, club_count=${club_count}, current_year=${current_year}, image_url=${image_url}, cgpa=${cgpa}, current_focus=${current_focus}, core_philosophy=${core_philosophy}, contact_text=${contact_text}, resume_url=${resume_url} WHERE id=${exists.rows[0].id};`;
        } else {
          await sql`INSERT INTO profile (name, subtitle, hero_badge, email, phone, linkedin, github, about_text, location, languages, project_count, club_count, current_year, image_url, cgpa, current_focus, core_philosophy, contact_text, resume_url) VALUES (${name}, ${subtitle}, ${hero_badge}, ${email}, ${phone}, ${linkedin}, ${github}, ${about_text}, ${location}, ${languages}, ${project_count}, ${club_count}, ${current_year}, ${image_url}, ${cgpa}, ${current_focus}, ${core_philosophy}, ${contact_text}, ${resume_url});`;
        }
